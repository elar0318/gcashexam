...
import static com.gcash.exam.delivery.model.response.VoucherResponse.getValidVoucherDiscount;
...
//        VoucherResponse response = restTemplate.getForObject(url, VoucherResponse.class, voucherCode);
        ...
        Optional<VoucherResponse> response = getValidVoucherDiscount(voucherCode);

        log.info("Success response from outbound API...");

        // Check if the voucher is present and not expired then return the discount rate
        if (response.isPresent() && !DateUtil.isExpired(response.get().expiry())) {
            log.info("Voucher found and not expired...Applying discount...");
            return applyDiscount(totalCostBeforeDisc, response.get().discount());
        } else {
            log.info("Invalid voucher or has expired");
